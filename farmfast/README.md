...
```mermaid
flowchart TD
    Farmer["📱 Farmer\n(WhatsApp)"] -->|"Photo + messages"| Twilio["Twilio\nWhatsApp Gateway"]
    Twilio -->|"POST webhook"| AppRunner

    Buyer["💻 Buyer\n(Browser)"] -->|"HTTPS"| CloudFront["Amazon CloudFront\nCDN + HTTPS"]
    CloudFront --> AppRunner["AWS App Runner\nNext.js 16 · Port 8080"]

    AppRunner -->|"Image upload\npre-signed URLs"| S3["Amazon S3\nProduce Images"]
    AppRunner -->|"AI grading\nHindi chat"| Bedrock["Amazon Bedrock\nNova Lite · Nova Micro · Nova Pro"]
    AppRunner -->|"All DB reads/writes\nRealtime subscriptions"| Supabase["Supabase\nPostgreSQL + Realtime"]
    AppRunner -->|"Offer alerts\nto farmer"| Twilio

    Bedrock -->|"Tool call:\nget_mandi_price"| Supabase

    CI["GitHub Actions\nCI/CD"] -->|"docker push"| ECR["Amazon ECR\nContainer Registry"]
    ECR -->|"AutoDeploy on\nnew image"| AppRunner
```
